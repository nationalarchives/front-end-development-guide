...
User focus not present on Education resources when tabbing

#### Developer(s) actions in response

This issue has now been fixed. Earlier radio buttons were used to show and hide the states of the but had to use the arrow navigation keys to access the `Education resourses`.
Now users can tab through without using the arrow keys.
...
1. NA
2. NA
4. JS is within an external file with
5. NA
...
Learnt more about accessibility. Even thought the previous itteration passed all accessibility criterias, the form factor of using radio buttons as tab tools it not appropriate for a visually impaired user.
