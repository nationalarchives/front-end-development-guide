...
* The PHP is beautifully formatted. I think we should update the peer-review-template.md to say that ‘source code is properly formatted’ - could you make this amendment? 
* The title tag doesn’t currently follow the ```{h1} | The National Archives``` pattern
* The ```lang``` attribute is currently ```en```. There’s nothing wrong with using just the ISO language code, but there would be better if we also included the ISO country code.
...
* There are some attributes which use camelCase and should be converted to dash-separated. i.e. iFrameRespo should be iframe-repo
...
* There is a very sensible document outline but I wonder if:
  * The ‘twitter’ badge needs a heading
  * The ‘latest activity’ heading needs to be elevated a level in the DOM (since it’s current position excludes some of its children). 
...
* There are quite a few WAI-ARIA landmark roles that could usefully be applied, including ‘banner’, ‘search’, ‘main’ and ‘navigation’. I also wonder if there’s a good use for ‘complementary’. 
